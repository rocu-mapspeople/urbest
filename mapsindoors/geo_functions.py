...
class GeoFunctions:
    ...
    def get_location_floor_index(self, location_id):
        ...
        if parent_object.baseType == 'floor':
            return int(parent_object.baseTypeProperties.administrativeid)
        elif location.baseType == 'venue':
            return 'Location is not inside of building polygon. Parent is venue.'



    def geodesic_point_buffer(self, lat:float, lon:float, m:float):
        """
        returns an array of coordinates for a circle based on a latitude, longitude and a radius for meters
    ...
    def place_2d_model_on_location(self, payload:list, bearer_token:str):
        """
        integration API service to rotate and scale an image over MapsIndoors locations.

        Parameters
        ----------
        payload --> list of dictionaries of MapsIndoors locations and the height & width ratio of the image
        for simplicity you can use the number of pixels wide and high to get the values for width and height.
        what's important is the aspect ratio.

        bearer token --> POST requests require a bearer token, use the OAuth class from the auth_client module to generate one.
        ...
        """
        response = requests.request("POST", url=self.url.place2dmodel(), headers={'Accept': 'application/json', 'Authorization': bearer_token}, json=payload)
